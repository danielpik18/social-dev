...
import styles from './AddEstudioDialog.module.scss';
...
const AddEstudioDialog = () => {
    const [stepDirection, setStepDirection] = useState('right');

    const { addEstudioDialogOpen, setAddEstudioDialogOpen } = useContext(EstudiosContext);
    const {
        errorDialog,
        setErrorDialog,
        activeStep,
    ...
    return (
        <Dialog
            open={addEstudioDialogOpen}
            onClose={() => setAddEstudioDialogOpen(false)}
        >
...
export default AddEstudioDialog;